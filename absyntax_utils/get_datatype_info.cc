...
bool get_datatype_info_c::is_type_equal(symbol_c *first_type, symbol_c *second_type)
{
    ...
    if ((is_ANY_generic_type( first_type)) ||
            (is_ANY_generic_type(second_type)))
    {
        return true;
    }
    ...
    if ((is_ANY_ELEMENTARY_compatible(first_type)) &&
            (typeid(*first_type) == typeid(*second_type)))
    {
        return true;
    }
    if (   is_ANY_ELEMENTARY_compatible(first_type)
            || is_ANY_ELEMENTARY_compatible(second_type))
    {
        return false;
    }
...
bool get_datatype_info_c::is_ref_to(symbol_c *type_symbol)
{
    ...
    {
        return true;   /* identifier ':' ref_spec_init */
    }
    if (typeid(*type_decl) == typeid(ref_spec_init_c))
    {
        return true;   /* ref_spec [ ASSIGN ref_initialization ]; */
    }
    if (typeid(*type_decl) == typeid(ref_spec_c))
    {
        return true;   /* REF_TO (non_generic_type_name | function_block_type_name) */
    }
    return false;
}
...
bool get_datatype_info_c::is_function_block(symbol_c *type_symbol)
{
    ...
    {
        return true;   /*  FUNCTION_BLOCK derived_function_block_name io_OR_other_var_declarations function_block_body END_FUNCTION_BLOCK */
    }
    return false;
}
...
bool get_datatype_info_c::is_subrange(symbol_c *type_symbol)
{
    ...
    {
        return true;   /*  subrange_type_name ':' subrange_spec_init */
    }
    if (typeid(*type_decl) == typeid(subrange_spec_init_c))
    {
        return true;   /* subrange_specification ASSIGN signed_integer */
    }
    if (typeid(*type_decl) == typeid(subrange_specification_c))
    {
        return true;   /*  integer_type_name '(' subrange')' */
    }

    if (typeid(*type_decl) == typeid(subrange_c))
    {
        ERROR;   /*  signed_integer DOTDOT signed_integer */
    }
    return false;
}
...
bool get_datatype_info_c::is_enumerated(symbol_c *type_symbol)
{
    ...
    {
        return true;   /* enumerated_specification ASSIGN enumerated_value */
    }
    if (typeid(*type_decl) == typeid(enumerated_value_list_c))
    {
        return true;
    }   /* enumerated_value_list ',' enumerated_value */        /* once we change the way we handle enums, this will probably become an ERROR! */

    if (typeid(*type_decl) == typeid(enumerated_value_c))
    {
        ERROR;   /* enumerated_type_name '#' identifier */
    }
    return false;
}
...
bool get_datatype_info_c::is_array(symbol_c *type_symbol)
{
    ...
    {
        return true;   /*  identifier ':' array_spec_init */
    }
    ...
    {
        return true;   /* ARRAY '[' array_subrange_list ']' OF non_generic_type_name */
    }

    if (typeid(*type_decl) == typeid(array_subrange_list_c))
    {
        ERROR;   /* array_subrange_list ',' subrange */
    }
    if (typeid(*type_decl) == typeid(array_initial_elements_list_c))
    {
        ERROR;
    }         /* array_initialization:  '[' array_initial_elements_list ']' */  /* array_initial_elements_list ',' array_initial_elements */
    if (typeid(*type_decl) == typeid(array_initial_elements_c))
    {
        ERROR;   /* integer '(' [array_initial_element] ')' */
    }
    return false;
}
...
bool get_datatype_info_c::is_structure(symbol_c *type_symbol)
{
    ...
    {
        return true;   /*  structure_type_name ':' structure_specification */
    }
    if (typeid(*type_decl) == typeid(initialized_structure_c))
    {
        return true;   /* structure_type_name ASSIGN structure_initialization */
    }
    if (typeid(*type_decl) == typeid(structure_element_declaration_list_c))
    {
        return true;
    }   /* structure_declaration:  STRUCT structure_element_declaration_list END_STRUCT */ /* structure_element_declaration_list structure_element_declaration ';' */

    if (typeid(*type_decl) == typeid(structure_element_declaration_c))
    {
        ERROR;   /*  structure_element_name ':' *_spec_init */
    }
    if (typeid(*type_decl) == typeid(structure_element_initialization_list_c))
    {
        ERROR;
    }         /* structure_initialization: '(' structure_element_initialization_list ')' */  /* structure_element_initialization_list ',' structure_element_initialization */
    if (typeid(*type_decl) == typeid(structure_element_initialization_c))
    {
        ERROR;   /*  structure_element_name ASSIGN value */
    }
    return false;
}
...
bool get_datatype_info_c::is_ANY_REAL_literal(symbol_c *type_symbol)
{
    if (type_symbol == NULL)
    {
        return true;   /* Please make sure things will work correctly before changing this to false!! */
    }
