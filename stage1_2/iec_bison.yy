 ...
 */
#define locloc(foo) foo.first_line,\
					foo.first_column,\
					foo.first_file,\
					foo.first_order,\
					foo.last_line,\
...
#define   locf(foo) foo.first_line,\
					foo.first_column,\
					foo.first_file,\
					foo.first_order

#define   locl(foo) foo.last_line,\
					foo.last_column,\
					foo.last_file,\
					foo.last_order
         ...
         do\
           if (N)\
             ...
             {\
               ...
               (Current).first_file   = (Current).last_file   = YYRHSLOC(Rhs, 0).last_file;\
               (Current).first_order  = (Current).last_order  = YYRHSLOC(Rhs, 0).last_order;\
             }\
         while (0)
...
| prev_declared_ref_type_name               {
												$$ = new identifier_c(((token_c *)$1)->value, locloc(@$));
												// change the derived_datatype_identifier_c into an identifier_c, 
												// as it will be taking the place of an identifier!
											} 
...
| prev_declared_derived_function_name       {
												$$ = new identifier_c(((token_c *)$1)->value, locloc(@$));
												// change the poutype_identifier_c into an identifier_c, 
												// as it will be taking the place of an identifier!
											}
...
integer_literal:
	...
	| integer_type_name signed_integer
		{
			$$ = NULL;
			print_err_msg(locl(@1), locf(@2), "'#' missing between integer type name and value in integer literal.");
			yynerrs++;
		}
	| integer_type_name binary_integer
		{
	...
	| integer_type_name '#' error
		{
			$$ = NULL; 
			if (is_current_syntax_token())
			{
				print_err_msg(locl(@2), locf(@3), "no value defined for integer literal.");
			}
			else
			{
				print_err_msg(locf(@3), locl(@3), "invalid value for integer literal."); 
				yyclearin;
			}
			yyerrok;
		};
...
| real_type_name signed_real
	{
		$$ = NULL; 
		print_err_msg(locl(@1), locf(@2), "'#' missing between real type name and value in real literal."); 
		yynerrs++;
	}
| real_type_name '#' error
	{
		...
		if (is_current_syntax_token()) 
		{
			print_err_msg(locl(@2), locf(@3), "no value defined for real literal.");
		}
		else 
		{
			print_err_msg(locf(@3), locl(@3), "invalid value for real literal."); 
			yyclearin;
		}
		yyerrok;
	}
